...
orth_Ttests <- orth_Ttests %>% mutate(P.vals = Corrected_orth$p_val_print,
                              ...
                              dplyr::select(!"Pr(>|z|)")
write.xlsx(bko2_orth_contrast_table, file = "bko2 tables/orth contrast.xlsx")

df1.to_excel("output.xlsx")  
orth_Ttests.to_excel("bko2 tables/orth t test table.xlsx")
...
#making a table with the corrected p values
vowel_Ttests <- vowel_Ttests %>% mutate(P.vals = Corrected_v$p_val_print,
                                      P.vals = as.numeric(P.vals),
                                      P.vals = round(P.vals, digits = 4),
                                      Estimate = round(Estimate, digits = 4),
                                      `Std. Error` = round(`Std. Error`, digits = 4),
...
model2 <- glmer(correct ~ Range * sound *voicing * congruent * vowel_type * consonant_type + (1|ID), data = all_data_df, family = "binomial")
Anova(model2)
#This model fails to converge, too many categories that are too overlapping 
```
