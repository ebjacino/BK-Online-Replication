...
This procedure requires you to have: the RDS files for each participant in bko2, an empty file for the CSVs, and empty file for practice trials. 
This file creates graphs and does analysis for each main measure in bko2.
options(digits = 3)
...
generate CSVs from RDS- only run if you have an empty folder ready for the csv to go 
...
for(file in rds_files){
  ...
  ##creating tidy data and choice bias score***
...
Graph for all the stimuli for all age groups individually
```{r}
...
```{r}
#stimulus:Range gets you the interaction term, which is the difference from 0 for each stimulus for each age range. (1|ID) is adding subject ID in as a random effect.
...
#16 stimuli * 3 different age ranges, multiplies every p value by 48
#sprintf prints out the more readable version of p value decimle 
#[,4] gets the 4th column
...
#ggsave("graphs/2023 poster/sig orthography.png", height = 3.5, width = 5.5)

```

how to tell if the age groups are different from each other with a contrast table
```{r}
...
bko2_orth_contrast_table <-contrast(emmeans(m_mult, ~ Range | sound) , method = "pairwise", type = "response")
bko2_orth_contrast_table <- as.data.frame(bko2_orth_contrast_table)
bko2_orth_contrast_table <- bko2_orth_contrast_table %>% mutate(
...
#write.csv(bko2_orth_contrast_table, file = "bko2 tables/orth contrast.xlsx")
```

#orthography IS THERE A SIGNIFICANT DIFFERENCE FROM 0?
 you want to see if every group is different from zero, and so you want a model that just looks at the smallest groups (each sound for each age group), with none of the big groupings. that’s why you just look at the interaction, not sound*range. This is doing those 6 individual t tests
 **important: we are not doing t tests here really, more like z tests? this is testing if the estimate of the interaction between sound and range is sig diff from chance (0)
```{r}
#checking 
age_group <- all_data_df %>% filter(Range == "6 to 8") %>% filter(sound == "curved")
# One-sample t-test
res <- t.test(age_group$shape_choice, mu = 0.5)
...
#multiply all the p values by 6- Bonferonni 
                 # p_vals = p.adjust(summary(stim_m)$coefficients[,4], method = "bonferroni", n = 16*3)


Corrected_orth <- tibble(categories = row.names(summary(stim_o)$coefficients),
                         p_vals = p.adjust(summary(stim_o)$coefficients[,4], method = "bonferroni", n = 2*3),
                         p_val_print = sprintf("%.10f", p_vals))
Corrected_orth
#All bars are significantly different from zero except curved 6 to 8, does not survive correction
```
#orthography t test table
```{r}
stim_o <- glmer(shape_choice ~ 0 + sound:Range + (1|ID), data = all_data_df, family = binomial)
summary(stim_o)
orth_summary <- summary(stim_o)
...
orth_Ttests <- as.data.frame(orth_summary$coefficients) 

#making a table with the corrected p values
orth_Ttests <- orth_Ttests %>% mutate(categories = row.names(orth_summary$coefficients),
                                      P.vals = Corrected_orth$p_val_print,
...
#write_csv(orth_Ttests, file = "bko2 tables/orth t test table.csv")
```
...
congruency_plot_new
#ggsave("graphs/2023 poster/congruency plot.png", height = 3.5, width = 5.5)
```
#congruency analysis
...
contrast(emmeans(m_mult_c, ~ Range | congruent) , method = "pairwise", type = "response") %>% confint()
...
bko2_congruency_contrast <-contrast(emmeans(congruency_i, ~ congruent | Range) , method = "pairwise", type = "response")
...
#odds ratio is the effect size- if there is no difference the odds ratio is equal to 1, no effect. If it's farther from 1 in either direction it's more significant
write.csv(bko2_congruency_contrast, file = "bko2 tables/congruency contrast.csv")
```
...
#Bonferroni correction
Corrected_v <- tibble(categories = row.names(summary(stim_v)$coefficients),
                         p_vals = p.adjust(summary(stim_v)$coefficients[,4], method = "bonferroni", n = 2*3),
                         p_val_print = sprintf("%.10f", p_vals))
Corrected_v
#vowel_type/a/:RangeAdult is the only one significantly different from chance
...
vowel_Ttests <- as.data.frame(vowel_summary$coefficients) 

#making a table with the corrected p values
vowel_Ttests <- vowel_Ttests %>% mutate(categories = row.names(vowel_summary$coefficients),
                                      P.vals = Corrected_v$p_val_print,
                                      P.vals = as.numeric(P.vals),
                                      P.vals = round(P.vals, digits = 4),
                                      Estimate = round(Estimate, digits = 4),
                                      `Std. Error` = round(`Std. Error`, digits = 4),
...
#write_csv(vowel_Ttests, file = "bko2 tables/vowel t test table.csv")
```
...
#Bonferroni correction
Corrected_cv <- tibble(categories = row.names(summary(cv_binom)$coefficients),
                         p_vals = p.adjust(summary(cv_binom)$coefficients[,4], method = "bonferroni", n = 2*3),
                         p_val_print = sprintf("%.10f", p_vals))
Corrected_cv
#only RangeAdult:voicingvoiced is significantly different from chance
```
#voicing t test table
```{r}
cv_binom <- glmer(shape_choice ~ 0 + Range:voicing + (1|ID), data = all_data_df, family = "binomial")
summary(cv_binom)
cv_summary <- summary(cv_binom)
...
voicing_Ttests <- as.data.frame(cv_summary$coefficients) 

#making a table with the corrected p values
voicing_Ttests <- voicing_Ttests %>% mutate(P.vals = Corrected_cv$p_val_print,
                                      P.vals = as.numeric(P.vals),
                                      P.vals = round(P.vals, digits = 4),
                                      Estimate = round(Estimate, digits = 4),
                                      `Std. Error` = round(`Std. Error`, digits = 4),
...
#write.csv(voicing_Ttests, file = "bko2 tables/voicing t test table.csv")
```
...
consonant_graph
#ggsave("graphs/2023 poster/consonant.png", height = 3.5, width = 5.5)
...
ct_real_binom <- glmer(shape_choice ~ 0 + Range:consonant_type + (1|ID), data = all_data_df, family = "binomial")
summary(ct_real_binom)

#Bonferroni correction
Corrected_ct <- tibble(categories = row.names(summary(ct_real_binom)$coefficients),
                         p_vals = p.adjust(summary(ct_real_binom)$coefficients[,4], method = "bonferroni", n = 3*3),
                         p_val_print = sprintf("%.10f", p_vals))
Corrected_ct
#only thing that changes is Range6 to 8:consonant_typestop is no longer significant
```
#consonant t test table
```{r}
ct_real_binom <- glmer(shape_choice ~ 0 + Range:consonant_type + (1|ID), data = all_data_df, family = "binomial")
summary(ct_real_binom)
c_summary <- summary(ct_real_binom)
...
consonant_Ttests <- as.data.frame(c_summary$coefficients) 

#making a table with the corrected p values
consonant_Ttests <- consonant_Ttests %>% mutate(P.vals = Corrected_ct$p_val_print,
                                      P.vals = as.numeric(P.vals),
                                      P.vals = round(P.vals, digits = 4),
                                      Estimate = round(Estimate, digits = 4),
                                      `Std. Error` = round(`Std. Error`, digits = 4),
                                      `z value` = round(`z value`, digits = 4)) %>%
                              dplyr::select(!"Pr(>|z|)")
#write.csv(consonant_Ttests, file = "bko2 tables/consonant type t test table.csv")
```
...
cor <- spcor.test(data_WJ$WJ_score,data_WJ$bias,data_WJ$Age,method="pearson")
report(cor)

#to see if there is an interaction between Age and WJ score? does the correlation between 2 variables change with age? the result with : in it is the interaction
#linear regression
...
#to see if there is an interaction between Age and WJ score? does the correlation between 2 variables change with age? the result with : in it is the interaction
#linear regression
WJ_lmk <- lm(data_WJ_kids$bias ~ data_WJ_kids$Age + data_WJ_kids$WJ_score + data_WJ_kids$Age*data_WJ_kids$WJ_score)
summary(WJ_lmk)
```

What is the range of scores for each group?
```{r}
#6to8
range(data_WJ_6to8$WJ_score)

#9to11
range(data_WJ_9to11$WJ_score)

#adult
range(data_WJ_adult$WJ_score)
```
...
model <- glmer(correct ~ Range * (sound +voicing + congruent + vowel_type + consonant_type) + (1|ID), data = all_data_df, family = "binomial")
Anova(model)
summary(model)
