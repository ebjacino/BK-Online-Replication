...
LG_bias_poirel <- LG_poirel_all %>% group_by(Range, LG_group) %>%
                                    summarize(mean_bias = mean(GminusL),
                                              #is this making it too small because it's divided by group? yes
                                    sd_bias = sd(GminusL),
                                    n = n(),
                                    se_bias=sd_bias/sqrt(n())) %>% ungroup()

## make a dataset with one row per person
...
LG_poirel_plot2 <-ggplot() +
  ...
  geom_jitter(BK_LG_scores, mapping = aes(x = Range, y = GminusL)) +
  theme_light() +
  ylab("Global Bias") +
