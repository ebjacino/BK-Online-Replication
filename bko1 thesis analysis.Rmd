...
bk_data_combined <- bk_data_combined %>% filter(!ID %in% first_25_adults)
...
bk_overall
#ggsave("graphs/thesis graphs/bk_overall.png", width = 10, height = 5)
```
#bk analyses
```{r}
#Different from zero?
...
bk_df_version <- bk_data_combined %>% filter(Range == "Adult")
bk_df_version <- bk_df_version %>% full_join(bk_version,bk_df_version, by = "ID")

bk_df_version_count <- bk_df_version %>% group_by(version,sound,Range,ID) %>% 
  summarise(bias = sum(shape_choice)/n() - 0.5) %>%
  group_by(sound, version, Range) %>%
...
cor.test(BK_LG_scores_5$GminusL, BK_LG_scores_5$total_bk_bias, method = "pearson")
#= 0.2555
...
LG_confound_grouped <- LG_confound_global_bias   %>%
  ...
  summarise(mean_bias = mean(globa_bias),
            sd_bias = sd(globa_bias),
            se_bias=sd_bias/sqrt(n()))
```
