...
#LG_list<- as.data.frame(unique(LG_data_df$ID)) #at 101 June 5
```
...
#BK_list<- as.data.frame(unique(bk_data_df$ID)) #at 101 June 5
...
#rep_bk_list<- as.data.frame(unique(bk_rep_df$ID)) #at 101 June 5
...
#import sheet 2 of bko1 demographics

LG_demographic_table <- bk_lg_demographics

#full_join to combine
LG_data_df_combined <- full_join(LG_data_df,target_table, by="image_ID_targets")
LG_data_df_combined <- left_join(LG_demographic_table, LG_data_df_combined, by = "ID")
...
LG_list<- as.data.frame(unique(LG_data_df_combined_bias$ID)) #at 101 June 5
...
#write.csv(LG_poirel_all, "full datasets/LG full dataset.csv")
```
...
####average GminusL for each age range
LG_poirel_plotviolin <-ggplot() +
  #geom_col(LG_weighted_avg, mapping = aes(x = Range, y = avg_bias), color = "black", fill = NA) +
  geom_violin(LG_bias_poirel_one_row, mapping = aes(x = Range, y = GminusL, fill = Range), alpha = 0.5) +
  #geom_errorbar(LG_weighted_avg, mapping = aes(x = Range, y = avg_bias, ymin = avg_bias-se_bias,
                                              #ymax = avg_bias+se_bias), alpha = 1, width = .75) +
  geom_point(LG_poirel_points, mapping = aes(x = Range, y = bias_round, size = Count, color = Range)) +
  theme_light() +
  ...
  xlab("global and local groups in each age range") +
  ylab("<- local bias, global bias ->")+
  stat_summary(data = LG_bias_poirel_one_row,mapping = aes(x = Range, y = GminusL), 
               fun.y  = "mean",
               geom = "crossbar", 
               width = 0.5,
               colour = "black")

LG_poirel_plotviolin

```
#lg graph global bias
```{r}
#to count choice bias and see how many participants are in each age range
LG_bias_data <- LG_data_df_combined_bias %>% group_by(ID,Range) %>% 
                                    summarise(global_bias = sum(choice_number)/n()) %>% 
...
#calculating the points that go on the graph
LG_points <- LG_data_df_combined_bias  %>% group_by(ID,Range) %>%
                        summarise(global_bias = sum(choice_number)/n()) %>% 
                        mutate(bias_round = round_any(global_bias,.25)) %>%  ungroup() %>%
                        group_by(Range,bias_round) %>%
                        summarise(Count = length(unique(ID)))

####average GminusL for each age range
LG_global_plot <-ggplot() +
  ...
  geom_errorbar(LG_bias_data, mapping = aes(x = Range, y = mean_bias, ymin = mean_bias-se_bias,
                                              ymax = mean_bias+se_bias), alpha = 1, width = .75) +
  geom_point(LG_points, mapping = aes(x = Range, y = bias_round, size = Count, color = Range)) +
  theme_light() +
  ylab("Global Bias") +
  #scale_color_manual(values = rev(cols)) +
  #scale_fill_manual(values = rev(cols)) +
  xlab("global and local groups in each age range") +
  ylab("Proportion of global responses")

LG_global_plot
```
#demographic analysis lg
```{r}
#make one data set with one row for each participant
LG_demo_df <- full_join(LG_bias_poirel_one_row,LG_demographic_table, by= "ID")
lg_demo_summedsex <- LG_demo_df %>% group_by(Sex) %>%
                      summarise(avg = mean(GminusL), 
                                sd = sd(GminusL),
                                se=sd/sqrt(n()), 
                                count = n())

#sex for each age range?
glm_sex <- glm(GminusL ~ Sex*Range.x, data = LG_demo_df)
Anova(glm_sex)

demosex <- t.test(GminusL ~ Sex, data = LG_demo_df)
report(demosex)

ggplot()+
  geom_bar(lg_demo_summedsex,mapping = aes(x = Sex, y = avg), stat = "identity")+
  theme_light()+
  #facet_wrap("Range.x")+
  geom_errorbar(lg_demo_summedsex, mapping = aes(x = Sex, y = avg, ymin = avg-se, ymax = avg+se), alpha = 1, width = 0.5) +
  geom_jitter(LG_demo_df, mapping = aes(x = Sex, y = GminusL))+
  ylab("<-local bias, global bias ->")

#race for each age range?
LG_demo_df <- LG_demo_df %>% mutate(Race = ifelse(Race == "White", "White", "nonWhite"))

glm_race <- glm(GminusL ~ Race*Range.x, data = LG_demo_df)
Anova(glm_race)
...
lgm1 <- lm(GminusL ~ 0 + Range, data = LG_bias_poirel_one_row) #one way anova. IV is categorical not continuous
summary(lgm1)
Anova(lgm1)
report(lgm1)
...
LG_bias_t_tests <- LG_bias_poirel_one_row %>% group_by(Range) %>%
                       summarise(t_val = t.test(GminusL, mu = 0)$statistic,
                                 df = t.test(GminusL, mu = 0)$parameter,
                                 p_val = t.test(GminusL, mu = 0)$p.value,
                                 hedgg = hedges_g(GminusL ~ 1)$Hedges_g)

LG_adults <- LG_bias_poirel_one_row %>% filter(Range == "Adult")
test1 <- t.test(LG_adults$GminusL, mu = 0)
test1
hg <- hedges_g(test1)
hg$Hedges_g

hedges_g(GminusL ~ 1, data = LG_adults)
effectsize(test1)
...
```{r}
LG_bias_poirel_one_row_forgraph <- LG_bias_poirel_one_row %>% mutate(LG_group_bi = ifelse(LG_group == "local", 1,0))

lgm2 <- glm(LG_group_bi ~ 0 + Range, data = LG_bias_poirel_one_row_forgraph, family = binomial)
summary(lgm2)
Anova(lgm2)
effectsize(lgm2)

ggplot(LG_bias_poirel_one_row_forgraph, aes(x = Range_Num, y = LG_group_bi, color = Range_Num))+
...
When target_number 6 or 9, it's many, and when it's 3 or 4, it's few
```{r}
#creates a count of g - l responses, puts them into categories based on what the end number is (local or global group)
#one row per participant
LG_data_df_combined_bias_summed_targ <- LG_data_df_combined_bias %>% mutate(targ_summed = ifelse(target_number <5, "3-4", "6-9"))

LG_targ_number_count <-LG_data_df_combined_bias_summed_targ %>% group_by(ID,targ_summed,Range) %>% summarise(global_bias = sum(choice_number)/n())

#average values!! average of GminusL for each age range################
LG_number_bias <- LG_data_df_combined_bias_summed_targ %>% group_by(ID,Range,targ_summed) %>%
                                      summarise(bias = sum(choice_number)/n()) %>%
                                    ungroup() %>%
                                    group_by(Range,targ_summed) %>%
...
bk_num_plot <- ggplot() +
  geom_col(LG_number_bias, mapping = aes(x = targ_summed, y = mean_bias), color = "black", fill = NA) +
  #geom_point(bk_rep_bias_data_points, mapping =aes(x = targ_summed, y = bias_round, size = Count, color= targ_summed)) +
  geom_col(LG_number_bias, mapping = aes(x = targ_summed, y = mean_bias, fill = targ_summed), alpha = 0.5) +
  geom_errorbar(LG_number_bias, mapping = aes(x = targ_summed, y = mean_bias, ymin = mean_bias-se_bias, ymax = mean_bias+se_bias), alpha = 1, width = 0.5) +
  scale_fill_manual(values = c(angular_color,curved_color)) +
  scale_color_manual(values = c(angular_color,curved_color)) +
  theme_light() +
  ylim(0,1) +
  ylab("P of global responses") +
  xlab("Number of local elements") +
  facet_wrap("Range")+
...
#analysis
lgm_numberelements <- glm(global_bias ~ targ_summed *Range, data = LG_targ_number_count)
Anova(lgm_numberelements)
```
...
#reload in demographics, sheet 1
rep_ages <- read_excel("bk rep ages.xlsx")
bk_version <- read_xlsx("bko1 version.xlsx")
bk_rep_demo <- bko1_demographics

bk_data_version <- left_join(bk_version, bk_data_df, by = "ID") #left join will make it only have adults, since that's whats in bko1 version
unique(bk_data_version$ID) #41
bk_adults_for_rep <- bk_data_version %>% filter(version == "online")
unique(bk_adults_for_rep$ID) #17

bk_adults_for_rep <- bk_adults_for_rep %>% dplyr::select(!"version")
bk_rep_df_combined <- rbind(bk_adults_for_rep, bk_rep_df)
bk_rep_df_combined <- na.omit(bk_rep_df_combined)
unique(bk_rep_df_combined$ID) #now at 20, adults only

#add in demo info to adult df
bk_rep_adult_complete <- left_join(bk_rep_df_combined, bko1_demographics, by = "ID")
unique(bk_rep_adult_complete$ID) #still at 20, adults only

#not get the kids from bk_data_df
bk_rep_data_combinedwithdemo <- left_join(bko1_demographics, bk_data_df, by = "ID")
#select all but the kids (this df doesnt have the 3 extra adults bko)
bk_rep_data_combinedwithdemo <- bk_rep_data_combinedwithdemo %>% filter(!Range == "Adult")
unique(bk_rep_data_combinedwithdemo$ID) #39 kids

bk_rep_data_combinedwithdemo <- rbind(bk_rep_data_combinedwithdemo, bk_rep_adult_complete)
unique(bk_rep_data_combinedwithdemo$ID) #all 64


rep_bk_list<- as.data.frame(unique(bk_rep_data_combinedwithdemo$ID)) #at 64 June 11
#write.csv(bk_rep_data_combinedwithdemo, "full datasets/bk replication full dataset.csv")

#by ID, not sound. not useful?
bk_rep_ind <- bk_rep_data_combinedwithdemo %>% group_by(ID) %>%
                  mutate(total_bk_bias = sum(shape_choice)/n()-.5) %>% dplyr::select(ID, total_bk_bias)  %>%
                filter(row_number()==1) %>% na.omit


bk_rep_bias_data <- bk_rep_data_combinedwithdemo %>% mutate(stimulus = toupper(sub(".wav","",sub("sound/","",stimulus)))) %>%
...
bk_rep_bias_data_points <- bk_rep_data_combinedwithdemo %>% mutate(stimulus = toupper(sub(".wav","",sub("sound/","",stimulus)))) %>%
                                      group_by(ID,sound,Range) %>% 
                                      summarise(bias = sum(shape_choice)/n() - 0.5) %>% 
                                      ungroup() %>%
...
```{r}
bk_rep_data_combinedwithdemo
glm_sex <- glmer(shape_choice ~ Sex + (1|ID), data = bk_rep_data_combinedwithdemo, family = "binomial")
Anova(glm_sex)

bk_racesimple <- bk_rep_data_combinedwithdemo %>% mutate(Race = ifelse(Race == "White", "White", "nonWhite"))

glm_race <- glmer(shape_choice ~ Race + (1|ID), data = bk_racesimple, family="poisson")
Anova(glm_race)
```
...
bk_r_m1 <- glmer(shape_choice ~ 0 + sound:Range + (1|ID), data = bk_rep_data_combinedwithdemo, family = binomial)
summary(bk_r_m1)
report(bk_r_m1)
...
bk_m2 <- glmer(shape_choice ~ Range*sound + (1|ID), data = bk_rep_data_combinedwithdemo, family = "binomial")
...
contrast <- emmeans(bk_m2, ~ Range | sound)

eff_size(contrast, sigma = sigma(bk_m2), edf = 124)
...
bk_bias_contrasts <- write.csv(bk_contrast_table, file = "bko1 tables/replications bk contrasts.csv")
```
...
bk_overall
ggsave("graphs/thesis graphs/bko1 replication.png", height = 3.5, width = 6.5)
...
#this id grouped by ID and not sound, so adults are super close to zero
BK_bias_only <- bk_data_combined %>% group_by(ID,Age) %>%
                  mutate(total_bk_bias = sum(correct)/n()) %>% dplyr::select(ID, total_bk_bias)  %>%
                filter(row_number()==1) %>% na.omit
...
cor_plot <-  ggplot(BK_LG_scores, aes(GminusL, total_bk_bias, color = Age))
cor_plot + geom_point() +
  theme_light()+
  #ylim(-0.50,0.50)+
...
#confound_list <- as.data.frame(unique(LG_data_df_c$ID))
```
...
res2 <- t.test(global_bias ~ target_confound, data = adult)
# Printing the results
res2 #results are nonsignificant, p = 0.9935
cohens_d(res1)
