...
stim_m <- glmer(correct ~ 0 + stimulus:Range + (1 | ID), data = all_data_df, family = binomial, nAGQ = 0)
summary(stim_m)
...
stim_o <- glmer(correct ~ 0 + sound:Range + (1|ID), data = all_data_df, family = binomial)
summary(stim_o)
...
stim_v <- glmer(correct ~ 0 + vowel_type:Range + (1|ID), data = all_data_df, family = binomial) 
summary(stim_v)
...
voicing_mean <- all_data_df %>%  group_by(voicing,ID,Range) %>% 
                                      summarise(bias = sum(correct)/n()-.5) %>% 
                                      mutate(bias = ifelse(voicing == "unvoiced",-1*bias,bias)) %>%
...
```{r}
#by range. is there a difference between bias for vowel  for different age ranges?
m_mult_cv <- glm(bias ~ Range*voicing, data = data_voicing)
m_add_cv <- glm(bias ~ Range+voicing, data = data_voicing)
#trying to find the model that best fits the data with the least number of terms. not over or under fit to the data. 
#IC tab multiplies the residuals by the number of terms, higher number is worse
...
contrast(emmeans(m_add_cv, ~ Range | voicing) , method = "pairwise", type = "response")
...
```{r}
magnitude_consonant_type <- all_data_df %>% group_by(ID,Range,consonant_type) %>%
...
#ben mess
real_binom <- glmer(correct ~ 0 + Range:consonant_type + (1|ID), data = all_data_df, family = "binomial")
summary(real_binom)
#plogis(0.3830)
