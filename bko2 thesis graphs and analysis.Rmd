...
stimulus_bias_data_ben <- all_data_df %>% mutate(stimulus = toupper(sub(".wav","",sub("sound/","",stimulus)))) %>%
                                      group_by(ID,stimulus,sound,Range) %>% 
                                      summarise(bias = sum(correct)/n() - 0.5) %>% 
                                      mutate(bias = ifelse(sound == "angular",-1*bias,bias))

stim_m <- glm(bias ~ 0 + stimulus:Range, data = stimulus_bias_data_ben)
summary(stim_m)
...
stim_v <- glm(bias ~ 0 + vowel_type:Range, data = data_vowel_type, family = binomial) #binomial makes them all nonsignificant
summary(stim_v)
...
#consonant graph- this one is in the style of Bottini et al 2019 with the proportion of spiky shapes

```{r}
consonant_graph_df <- data_consonant_type %>% group_by(consonant_type,Range) %>%
                                      summarize(mean_bias = mean(bias),
                                                sd_bias = sd(bias),
                                                se_bias=sd_bias/sqrt(n()), 
                                                N = n()) 

consonant_graph_points <- data_consonant_type %>% mutate(bias_round = round_any(bias,0.25)) %>%
...
```{r}
#not sure if i should split fricatives (*them by -1)
data_split_consonant_type = all_data_df_c %>% group_by(consonant_type, ID,Range) %>%
                                      summarise(bias = sum(correct)/n()- 0.5) %>%  
                                      mutate(bias = ifelse(consonant_type == "stop",-1*bias,bias))
...
stim_c <- glm(bias ~ 0 + consonant_type:Range, data = data_split_consonant_type)
summary(stim_c)
...
ggplot()+
  geom_point(data = data_split_consonant_type %>% filter(Range == "Adult") %>% filter(consonant_type == "stop"),
             aes(y = bias, x = consonant_type), color = "blue", alpha = 0.25)+
  geom_point(data = data_consonant_type %>% filter(Range == "Adult") %>% filter(consonant_type == "stop"),
             aes(y = bias, x = consonant_type), color = "red", alpha = 0.25)+
  theme_classic()

#                                     Estimate Std. Error z value Pr(>|z|) 
#consonant_typestop:RangeAdult         0.54406    0.45268   1.202  0.22942

mean(consonant_data_adults_stop$bias)
sd(consonant_data_adults_stop$bias)/sqrt(length(consonant_data_adults_stop$bias))

#check t tests for adults
# One-sample t-test
consonant_data_adults_stop <- data_split_consonant_type %>% filter(Range == "Adult") %>% filter(consonant_type == "stop")

c_adult <- t.test(consonant_data_adults_stop$bias, mu = 0)
# Printing the results
c_adult

#check t tests for adults- sonorants
# One-sample t-test
consonant_data_adults_son <- data_split_consonant_type %>% filter(Range == "Adult") %>% filter(consonant_type == "sonorant")

c_adult <- t.test(consonant_data_adults_son$bias, mu = 0)
# Printing the results
c_adult

#check t tests for adults- fricatives
# One-sample t-test
consonant_data_adults_fr <- data_split_consonant_type %>% filter(Range == "Adult") %>% filter(consonant_type == "fricative")

c_adult <- t.test(consonant_data_adults_fr$bias, mu = 0)
# Printing the results
c_adult
```

is there a difference in the predictive power of the consonants and vowels? do they impact the chance the participant will choose spiky or round shape?
```{r}
